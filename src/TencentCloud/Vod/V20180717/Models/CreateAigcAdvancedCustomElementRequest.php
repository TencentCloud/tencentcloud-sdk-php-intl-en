 ...
 * @method string getReferenceType() Obtain <p>Subject reference method. The availability of subjects customized via video and via image is different.</p><p>Enumeration values:</p><ul><li>video_refer: Video role subject. At this point, refer to element_video_list to define the subject appearance.</li><li>image_refer: Multi-image subject. At this point, refer to element_image_list to define the subject appearance.</li></ul>
 * @method void setReferenceType(string $ReferenceType) Set <p>Subject reference method. The availability of subjects customized via video and via image is different.</p><p>Enumeration values:</p><ul><li>video_refer: Video role subject. At this point, refer to element_video_list to define the subject appearance.</li><li>image_refer: Multi-image subject. At this point, refer to element_image_list to define the subject appearance.</li></ul>
 * @method string getElementVoiceId() Obtain <p>Entity voice type, can bind existing timbres in the timbre library.</p><ul><li>When the current parameter is empty, the current entity is not bound to a timbre.</li><li>Only entities customized for video support binding timbres.</li></ul>
 * @method void setElementVoiceId(string $ElementVoiceId) Set <p>Entity voice type, can bind existing timbres in the timbre library.</p><ul><li>When the current parameter is empty, the current entity is not bound to a timbre.</li><li>Only entities customized for video support binding timbres.</li></ul>
 ...
 * @method void setElementVideoList(string $ElementVideoList) Set <p>Entity reference video, used to set the entity and its details through video.</p><ul><li>Videos with audio can be uploaded. If the video contains voice, it triggers custom timbre (customize + add to timbre library + bind with entity).</li><li>Required when referencing a video, unavailable when referencing an image.</li><li>Carried in key:value format, as follows:<br><pre><code>{  &quot;refer_videos&quot;:[    {      &quot;video_url&quot;:&quot;video_url_1&quot;    }  ]}</code></pre>● Only MP4/MOV video formats are supported.<br>● Only 1080P videos with a duration between 3s and 8s and an aspect ratio of 16:9 or 9:16 are supported.<br>● Up to 1 video can be uploaded, with a video size no more than 200MB.<br>● The video_url parameter value cannot be empty.</li></ul>
 * @method string getElementImageList() Obtain <p>Subject reference image. You can set the subject and its details through multiple images.</p><ul><li>Including a front reference image and other angle or close-up reference images. Among them:<ul><li>At least 1 front reference image, defined by the frontal_image parameter.</li><li>1–3 other reference images, which must differ from the front reference image, defined by the image_url parameter.</li></ul></li><li>Use key:value to carry them, as follows:<br><pre><code>{  "frontal_image":"image_url_0",  "refer_images":[    {      "image_url":"image_url_1"    },    {      "image_url":"image_url_2"    },    {      "image_url":"image_url_3"    }  ]}</code></pre></li></ul>
 * @method void setElementImageList(string $ElementImageList) Set <p>Subject reference image. You can set the subject and its details through multiple images.</p><ul><li>Including a front reference image and other angle or close-up reference images. Among them:<ul><li>At least 1 front reference image, defined by the frontal_image parameter.</li><li>1–3 other reference images, which must differ from the front reference image, defined by the image_url parameter.</li></ul></li><li>Use key:value to carry them, as follows:<br><pre><code>{  "frontal_image":"image_url_0",  "refer_images":[    {      "image_url":"image_url_1"    },    {      "image_url":"image_url_2"    },    {      "image_url":"image_url_3"    }  ]}</code></pre></li></ul>
 * @method string getTagList() Obtain <p>Configure tags for a principal. A principal can be configured with multiple tags.</p><ul><li>Use key:value to carry them. Among them, details are given below:</li></ul><p><pre><code>[  {        &quot;tag_id&quot;: &quot;o_101&quot;  }, {        &quot;tag_id&quot;: &quot;o_102&quot;    }]</code></pre></p>
 * @method void setTagList(string $TagList) Set <p>Configure tags for a principal. A principal can be configured with multiple tags.</p><ul><li>Use key:value to carry them. Among them, details are given below:</li></ul><p><pre><code>[  {        &quot;tag_id&quot;: &quot;o_101&quot;  }, {        &quot;tag_id&quot;: &quot;o_102&quot;    }]</code></pre></p>
...
class CreateAigcAdvancedCustomElementRequest extends AbstractModel
{
    ...
    /**
     * @var string <p>Subject reference method. The availability of subjects customized via video and via image is different.</p><p>Enumeration values:</p><ul><li>video_refer: Video role subject. At this point, refer to element_video_list to define the subject appearance.</li><li>image_refer: Multi-image subject. At this point, refer to element_image_list to define the subject appearance.</li></ul>
     */
    public $ReferenceType;

    /**
     * @var string <p>Entity voice type, can bind existing timbres in the timbre library.</p><ul><li>When the current parameter is empty, the current entity is not bound to a timbre.</li><li>Only entities customized for video support binding timbres.</li></ul>
     */
    public $ElementVoiceId;

    /**
     * @var string <p>Entity reference video, used to set the entity and its details through video.</p><ul><li>Videos with audio can be uploaded. If the video contains voice, it triggers custom timbre (customize + add to timbre library + bind with entity).</li><li>Required when referencing a video, unavailable when referencing an image.</li><li>Carried in key:value format, as follows:<br><pre><code>{  &quot;refer_videos&quot;:[    {      &quot;video_url&quot;:&quot;video_url_1&quot;    }  ]}</code></pre>● Only MP4/MOV video formats are supported.<br>● Only 1080P videos with a duration between 3s and 8s and an aspect ratio of 16:9 or 9:16 are supported.<br>● Up to 1 video can be uploaded, with a video size no more than 200MB.<br>● The video_url parameter value cannot be empty.</li></ul>
     */
    public $ElementVideoList;

    /**
     * @var string <p>Subject reference image. You can set the subject and its details through multiple images.</p><ul><li>Including a front reference image and other angle or close-up reference images. Among them:<ul><li>At least 1 front reference image, defined by the frontal_image parameter.</li><li>1–3 other reference images, which must differ from the front reference image, defined by the image_url parameter.</li></ul></li><li>Use key:value to carry them, as follows:<br><pre><code>{  "frontal_image":"image_url_0",  "refer_images":[    {      "image_url":"image_url_1"    },    {      "image_url":"image_url_2"    },    {      "image_url":"image_url_3"    }  ]}</code></pre></li></ul>
     */
    public $ElementImageList;

    /**
     * @var string <p>Configure tags for a principal. A principal can be configured with multiple tags.</p><ul><li>Use key:value to carry them. Among them, details are given below:</li></ul><p><pre><code>[  {        &quot;tag_id&quot;: &quot;o_101&quot;  }, {        &quot;tag_id&quot;: &quot;o_102&quot;    }]</code></pre></p>
     */
    public $TagList;
     ...
     * @param string $ReferenceType <p>Subject reference method. The availability of subjects customized via video and via image is different.</p><p>Enumeration values:</p><ul><li>video_refer: Video role subject. At this point, refer to element_video_list to define the subject appearance.</li><li>image_refer: Multi-image subject. At this point, refer to element_image_list to define the subject appearance.</li></ul>
     * @param string $ElementVoiceId <p>Entity voice type, can bind existing timbres in the timbre library.</p><ul><li>When the current parameter is empty, the current entity is not bound to a timbre.</li><li>Only entities customized for video support binding timbres.</li></ul>
     * @param string $ElementVideoList <p>Entity reference video, used to set the entity and its details through video.</p><ul><li>Videos with audio can be uploaded. If the video contains voice, it triggers custom timbre (customize + add to timbre library + bind with entity).</li><li>Required when referencing a video, unavailable when referencing an image.</li><li>Carried in key:value format, as follows:<br><pre><code>{  &quot;refer_videos&quot;:[    {      &quot;video_url&quot;:&quot;video_url_1&quot;    }  ]}</code></pre>● Only MP4/MOV video formats are supported.<br>● Only 1080P videos with a duration between 3s and 8s and an aspect ratio of 16:9 or 9:16 are supported.<br>● Up to 1 video can be uploaded, with a video size no more than 200MB.<br>● The video_url parameter value cannot be empty.</li></ul>
     * @param string $ElementImageList <p>Subject reference image. You can set the subject and its details through multiple images.</p><ul><li>Including a front reference image and other angle or close-up reference images. Among them:<ul><li>At least 1 front reference image, defined by the frontal_image parameter.</li><li>1–3 other reference images, which must differ from the front reference image, defined by the image_url parameter.</li></ul></li><li>Use key:value to carry them, as follows:<br><pre><code>{  "frontal_image":"image_url_0",  "refer_images":[    {      "image_url":"image_url_1"    },    {      "image_url":"image_url_2"    },    {      "image_url":"image_url_3"    }  ]}</code></pre></li></ul>
     * @param string $TagList <p>Configure tags for a principal. A principal can be configured with multiple tags.</p><ul><li>Use key:value to carry them. Among them, details are given below:</li></ul><p><pre><code>[  {        &quot;tag_id&quot;: &quot;o_101&quot;  }, {        &quot;tag_id&quot;: &quot;o_102&quot;    }]</code></pre></p>
