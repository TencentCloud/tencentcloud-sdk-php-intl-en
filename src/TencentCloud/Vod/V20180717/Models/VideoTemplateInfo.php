 ...
 *
 * @method string getCodec() Obtain <p>Video stream encoding format. Available values:</p><li>libx264: H.264 code;</li><li>libx265: H.265 code;</li><li>av1: AOMedia Video 1 code;</li><li>H.266: H.266 code.</li><font color="red">Note:</font><li>av1 and H.266 encoding containers currently only support mp4;</li><li>H.266 currently only supports fixed CRF bitrate control method.</li>
 * @method void setCodec(string $Codec) Set <p>Video stream encoding format. Available values:</p><li>libx264: H.264 code;</li><li>libx265: H.265 code;</li><li>av1: AOMedia Video 1 code;</li><li>H.266: H.266 code.</li><font color="red">Note:</font><li>av1 and H.266 encoding containers currently only support mp4;</li><li>H.266 currently only supports fixed CRF bitrate control method.</li>
 * @method integer getFps() Obtain <p>Video frame rate. Value ranges from 0 to 100. Measurement unit: Hz.<br>When the value is 0, it means the frame rate is consistent with the original video.</p>
 * @method void setFps(integer $Fps) Set <p>Video frame rate. Value ranges from 0 to 100. Measurement unit: Hz.<br>When the value is 0, it means the frame rate is consistent with the original video.</p>
 * @method integer getBitrate() Obtain <p>Bitrate of video stream, value ranges from 0 to [128, 100000], unit: kbps.<br>When the value is 0, it means the bitrate is automatically set by VOD.</p>
 * @method void setBitrate(integer $Bitrate) Set <p>Bitrate of video stream, value ranges from 0 to [128, 100000], unit: kbps.<br>When the value is 0, it means the bitrate is automatically set by VOD.</p>
 ...
 * @method string getPreserveHDRSwitch() Obtain <p>Whether the transcoding output still keeps HDR when the original video is HDR (High Dynamic Range). Value ranges from:</p><li>ON: If the raw file is HDR, the transcoding output keeps HDR; otherwise, the transcoding output is SDR (Standard Dynamic Range).</li><li>OFF: Regardless of whether the raw file is HDR or SDR, the transcoding output is SDR.</li>Default value: OFF.
...
{
    /**
     * @var string <p>Video stream encoding format. Available values:</p><li>libx264: H.264 code;</li><li>libx265: H.265 code;</li><li>av1: AOMedia Video 1 code;</li><li>H.266: H.266 code.</li><font color="red">Note:</font><li>av1 and H.266 encoding containers currently only support mp4;</li><li>H.266 currently only supports fixed CRF bitrate control method.</li>
     */
    public $Codec;

    /**
     * @var integer <p>Video frame rate. Value ranges from 0 to 100. Measurement unit: Hz.<br>When the value is 0, it means the frame rate is consistent with the original video.</p>
     */
    public $Fps;

    /**
     * @var integer <p>Bitrate of video stream, value ranges from 0 to [128, 100000], unit: kbps.<br>When the value is 0, it means the bitrate is automatically set by VOD.</p>
     */
    public $Bitrate;
    ...
    /**
     * @var string <p>Whether the transcoding output still keeps HDR when the original video is HDR (High Dynamic Range). Value ranges from:</p><li>ON: If the raw file is HDR, the transcoding output keeps HDR; otherwise, the transcoding output is SDR (Standard Dynamic Range).</li><li>OFF: Regardless of whether the raw file is HDR or SDR, the transcoding output is SDR.</li>Default value: OFF.
     */
    ...
    /**
     * @var integer <p>Number of B-frames between reference frames, set to auto by default.</p><p>Value ranges from 0 to 16.</p>
     */
    public $Bframes;

    /**
     * @var integer <p>Average duration of slices. 0 or empty means auto, which automatically chooses appropriate segment duration based on video features such as GOP.</p><p>Value ranges from 0 to 10.</p><p>Unit: second.</p><p>Supports only transcoding template. Adaptive bitrate stream template is not currently supported.</p>
     */
    public $HlsTime;

    /**
     * @var string <p>A predefined group of encoding tools or features allowed by video encoding standards, suitable for different scenarios.</p><p>Enumeration value:</p><ul><li>baseline: Supports only I/P frames and non-interlaced scenarios, suitable for video calls and mobile video.</li><li>main: Mainstream profile, provides I, P, and B frames, and supports both interlaced and non-interlaced modes. Mainly used in mainstream audio and video consumption products such as video players and streaming media transmission devices.</li><li>high: Highest encoding level, adds 8X8 prediction to the Main Profile and supports custom quantification. Widely used in Blu-ray storage and HDTV scenarios.</li><li>default: Automatic filling along with the original video.</li></ul><p>Default value: default</p><p>This configuration item is valid only when Codec is libx264.</p>
     */
    public $VideoProfile;

    /**
     * @param string $Codec <p>Video stream encoding format. Available values:</p><li>libx264: H.264 code;</li><li>libx265: H.265 code;</li><li>av1: AOMedia Video 1 code;</li><li>H.266: H.266 code.</li><font color="red">Note:</font><li>av1 and H.266 encoding containers currently only support mp4;</li><li>H.266 currently only supports fixed CRF bitrate control method.</li>
     * @param integer $Fps <p>Video frame rate. Value ranges from 0 to 100. Measurement unit: Hz.<br>When the value is 0, it means the frame rate is consistent with the original video.</p>
     * @param integer $Bitrate <p>Bitrate of video stream, value ranges from 0 to [128, 100000], unit: kbps.<br>When the value is 0, it means the bitrate is automatically set by VOD.</p>
     ...
     * @param integer $Bframes <p>Number of B-frames between reference frames, set to auto by default.</p><p>Value ranges from 0 to 16.</p>
     * @param integer $HlsTime <p>Average duration of slices. 0 or empty means auto, which automatically chooses appropriate segment duration based on video features such as GOP.</p><p>Value ranges from 0 to 10.</p><p>Unit: second.</p><p>Supports only transcoding template. Adaptive bitrate stream template is not currently supported.</p>
     * @param string $VideoProfile <p>A predefined group of encoding tools or features allowed by video encoding standards, suitable for different scenarios.</p><p>Enumeration value:</p><ul><li>baseline: Supports only I/P frames and non-interlaced scenarios, suitable for video calls and mobile video.</li><li>main: Mainstream profile, provides I, P, and B frames, and supports both interlaced and non-interlaced modes. Mainly used in mainstream audio and video consumption products such as video players and streaming media transmission devices.</li><li>high: Highest encoding level, adds 8X8 prediction to the Main Profile and supports custom quantification. Widely used in Blu-ray storage and HDTV scenarios.</li><li>default: Automatic filling along with the original video.</li></ul><p>Default value: default</p><p>This configuration item is valid only when Codec is libx264.</p>
     */
