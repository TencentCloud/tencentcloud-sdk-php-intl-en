 ...
 * @method integer getNeedConditionValue() Obtain Whether to return filter criteria. 0 for no, 1 for yes. Default is no.
 * @method void setNeedConditionValue(integer $NeedConditionValue) Set Whether to return filter criteria. 0 for no, 1 for yes. Default is no.
 * @method Conditions getConditions() Obtain Filter criteria. It only supports ResourceKeyword (resource keyword, which supports fuzzy query by resource ID or resource name), ProjectIds (project ID), RegionIds (region ID), PayModes (payment mode, prePay or postPay), HideFreeCost (whether to hide zero-amount transactions, 0 or 1), and OrderByCost (sorting rule by fees, desc or asc).
 * @method void setConditions(Conditions $Conditions) Set Filter criteria. It only supports ResourceKeyword (resource keyword, which supports fuzzy query by resource ID or resource name), ProjectIds (project ID), RegionIds (region ID), PayModes (payment mode, prePay or postPay), HideFreeCost (whether to hide zero-amount transactions, 0 or 1), and OrderByCost (sorting rule by fees, desc or asc).
 */
class DescribeCostSummaryByResourceRequest extends AbstractModel
{
    ...
    /**
     * @var integer Whether to return filter criteria. 0 for no, 1 for yes. Default is no.
     */
    public $NeedConditionValue;

    /**
     * @var Conditions Filter criteria. It only supports ResourceKeyword (resource keyword, which supports fuzzy query by resource ID or resource name), ProjectIds (project ID), RegionIds (region ID), PayModes (payment mode, prePay or postPay), HideFreeCost (whether to hide zero-amount transactions, 0 or 1), and OrderByCost (sorting rule by fees, desc or asc).
     */
    public $Conditions;
     ...
     * @param integer $NeedConditionValue Whether to return filter criteria. 0 for no, 1 for yes. Default is no.
     * @param Conditions $Conditions Filter criteria. It only supports ResourceKeyword (resource keyword, which supports fuzzy query by resource ID or resource name), ProjectIds (project ID), RegionIds (region ID), PayModes (payment mode, prePay or postPay), HideFreeCost (whether to hide zero-amount transactions, 0 or 1), and OrderByCost (sorting rule by fees, desc or asc).
     */
