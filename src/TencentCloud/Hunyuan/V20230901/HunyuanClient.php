 ...
 * @method Models\DescribeAutoRiggingJobResponse DescribeAutoRiggingJob(Models\DescribeAutoRiggingJobRequest $req) 3D Model Auto Rigging API, powered by the Hunyuan Large Model, processes input 3D models of characters or animals to perform automatic rigging and skinning, outputting a 3D model complete with skeletal data. By default, one concurrent slot is provided, meaning that a maximum of one submitted task can be processed simultaneously; the next task can only begin processing once the previous one has been completed.
 * @method Models\DescribeConvert3DFormatJobResponse DescribeConvert3DFormatJob(Models\DescribeConvert3DFormatJobRequest $req) According to Convert3DFormatJob, querying the specific results of the model format conversion typically requires the unique JobID corresponding to that task.
 * @method Models\DescribeHunyuanTo3DUVJobResponse DescribeHunyuanTo3DUVJob(Models\DescribeHunyuanTo3DUVJobRequest $req) Query component splitting tasks.
 ...
 * @method Models\QueryHunyuanTo3DTextureEditJobResponse QueryHunyuanTo3DTextureEditJob(Models\QueryHunyuanTo3DTextureEditJobRequest $req) Query 3D texture edit tasks, redraw the texture of the 3D model based on semantics or images, and output a redrawn texture image.
...
1 concurrent is provided by default, which means 1 submitted task can be processed simultaneously. The next task can be processed only after the previous task is completed.
 * @method Models\SubmitAutoRiggingJobResponse SubmitAutoRiggingJob(Models\SubmitAutoRiggingJobRequest $req) 3D Model Auto Rigging API, powered by the HY Large Model, processes input 3D models of characters or animals to perform automatic rigging and skinning, outputting a 3D model complete with skeletal data. By default, one concurrent slot is provided, meaning that a maximum of one submitted task can be processed simultaneously; the next task can only begin processing once the previous one has been completed.
 * @method Models\SubmitConvert3DFormatJobResponse SubmitConvert3DFormatJob(Models\SubmitConvert3DFormatJobRequest $req) After inputting the 3D model file, the system will automatically convert it to the required format. The maximum supported file size is 200MB.
...
This API is used to provide 1 concurrent task by default, which means only 1 submitted task can be processed simultaneously. The next task can be processed only after the previous task is completed.
 * @method Models\SubmitHunyuanTo3DTextureEditJobResponse SubmitHunyuanTo3DTextureEditJob(Models\SubmitHunyuanTo3DTextureEditJobRequest $req) After inputting the 3D model, redraw the texture of the 3D model based on semantics or images, and output a redrawn texture image.
 * @method Models\SubmitHunyuanTo3DUVJobResponse SubmitHunyuanTo3DUVJob(Models\SubmitHunyuanTo3DUVJobRequest $req) After inputting the model, UV unfolding can be performed based on the model texture to output the corresponding UV texture
 */
