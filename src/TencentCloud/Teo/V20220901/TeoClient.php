 ...
 * @method Models\ConfirmOriginACLUpdateResponse ConfirmOriginACLUpdate(Models\ConfirmOriginACLUpdateRequest $req) This API is used to confirm that the latest origin ACLs have been updated to the origin server firewall when the origin ACLs change. After confirming the update to the latest version, related change notifications will stop pushing.
 ...
 * @method Models\CreateWebSecurityTemplateResponse CreateWebSecurityTemplate(Models\CreateWebSecurityTemplateRequest $req) This API is used to create a security policy configuration template.
 ...
 * @method Models\DeleteWebSecurityTemplateResponse DeleteWebSecurityTemplate(Models\DeleteWebSecurityTemplateRequest $req) This API is used to delete a security policy configuration template.
 ...
 * @method Models\DescribeWebSecurityTemplateResponse DescribeWebSecurityTemplate(Models\DescribeWebSecurityTemplateRequest $req) This API is used to query security policy configuration template details.
 * @method Models\DescribeWebSecurityTemplatesResponse DescribeWebSecurityTemplates(Models\DescribeWebSecurityTemplatesRequest $req) This API is used to query the security policy configuration template list.
 ...
 * @method Models\DisableOriginACLResponse DisableOriginACL(Models\DisableOriginACLRequest $req) This API is used to disable 'Origin Protection' of a site. Once disabled, resources related to it will no longer use only the origin ACLs provided by "origin protection" to request your origin, and stops sending update notifications on the origin ACLs.
 ...
 * @method Models\EnableOriginACLResponse EnableOriginACL(Models\EnableOriginACLRequest $req) This API is used to enable 'Origin Protection' for Layer 4 or Layer 7 instances. The number of enabled instances has an upper limit: 200 for Layer 7 domains and 100 for Layer 4 proxy instances. The total number of instances cannot exceed 200, otherwise an error reminder will be triggered. You can first enable the maximum allowed number and use the ModifyOriginACL API to set the excess quantity.
 ...
 * @method Models\ModifyOriginACLResponse ModifyOriginACL(Models\ModifyOriginACLRequest $req) This API is used to enable or disable specific origin ACLs for L7 acceleration domain names or L4 proxy instances. A single submission supports up to 200 L7 acceleration domain names or 100 L4 proxy instances. Hybrid submissions of L7 acceleration domain names and L4 proxy instances are supported, with a maximum total number of instances of 200. If changes are needed for exceeding 200 instances, submit them in batches via this API.
 ...
 * @method Models\ModifyWebSecurityTemplateResponse ModifyWebSecurityTemplate(Models\ModifyWebSecurityTemplateRequest $req) This API is used to modify the security policy configuration template.
